...
func populateMap(m map[groupVersionKindName]newNameObject, obj runtime.Object, newName string) error {
	...
	if _, found := m[gvkn]; found {
		return fmt.Errorf("cannot use a duplicate name %q for %s: %v", oldName, gvk)
	}
...
func constructSecret(secret manifest.Secret) (*corev1.Secret, error) {
	...
	corev1secret.StringData = map[string]string{}
	var err error
	switch secret.Type {
	case "tls":
	...
	case "env":
		if err := configmapandsecret.HandleFromEnvFileSource(corev1secret, secret.EnvSource); err != nil {
			return nil, err
		}
	case "file":
		if err := configmapandsecret.HandleFromFileSources(corev1secret, secret.FileSources); err != nil {
			return nil, err
		}
	case "literal":
		if err := configmapandsecret.HandleFromLiteralSources(corev1secret, secret.LiteralSources); err != nil {
			return nil, err
		}
	default:
